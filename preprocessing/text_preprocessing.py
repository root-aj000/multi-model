...
try:
    ...
    # This tokenizer knows about BERT's vocabulary and special tokens
...
def tokenize_text(text, max_length=DEFAULT_MAX_LENGTH):
    ...
    try:
        ...
        # *** FIXED: BatchEncoding is a valid dictionary-like object ***
        # The tokenizer returns BatchEncoding which behaves like a dict
        # We can check if it has the required keys instead of type checking
        try:
...
if __name__ == "__main__":
    ...
    # ------------------------------------------------------------------------
    # USE CASE 4: Tokenization with Different Lengths
    # ------------------------------------------------------------------------
    print("USE CASE 4: Tokenization with different max_length values")
    print("-" * 70)
    
    try:
        test_text = "this is a sample text for testing tokenization"
        print(f"Text: '{test_text}'")
        print()
        
        # Try different max_length values
        for max_len in [16, 32, 64]:
            tokens = tokenize_text(test_text, max_length=max_len)
            
            if tokens is not None:
                real_tokens = tokens['attention_mask'].sum().item()
                padding_tokens = max_len - real_tokens
                
                print(f"max_length={max_len}:")
                print(f"  Shape: {tokens['input_ids'].shape}")
                print(f"  Real tokens: {real_tokens}, Padding: {padding_tokens}")
            else:
    ...
    # ------------------------------------------------------------------------
    # USE CASE 5: Complete Pipeline (Clean + Tokenize)
    # ------------------------------------------------------------------------
    print("USE CASE 5: Complete preprocessing pipeline")
    print("-" * 70)
